...
async def get_dashboard_stats(
    ...
    if current_user.role in [UserRole.ADMIN, UserRole.SUPERADMIN, UserRole.STAFF]:
        ...
        ).order_by(Appointment.scheduled_time).limit(5).all()
            ...
            "upcoming_appointments": [
                {
                    ...
                    "type": a.appointment_type if hasattr(a, 'appointment_type') else "consultation"
                }
                for a in upcoming
            ]
        }
        ...
        return {
            ...
            "next_appointment": {
                ...
                "scheduled_time": next_apt.scheduled_time.isoformat(),
                "type": next_apt.appointment_type if hasattr(next_apt, 'appointment_type') else "consultation"
    ...
    if period == "week":
        ...
        for i in range(6, -1, -1):
            ...
            day_completed = db.query(Appointment).filter(
                and_(
                    func.date(Appointment.scheduled_time) == day,
                    Appointment.status == AppointmentStatus.COMPLETED
                )
            ).count()
        ...
        for i in range(4):
            ...
            week_completed = db.query(Appointment).filter(
                and_(
                    func.date(Appointment.scheduled_time) >= week_start,
                    func.date(Appointment.scheduled_time) <= week_end,
                    Appointment.status == AppointmentStatus.COMPLETED
                )
            ).count()
        ...
        for i in range(11, -1, -1):
            ...
            # This is simplified - in production you'd use proper month boundaries
...
async def get_department_stats(
    ...
    for (spec,) in specializations:
        ...
        doctor_ids = [d.id for d in db.query(Doctor.id).filter(Doctor.specialization == spec).all()]
